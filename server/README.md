...
클라이언트가 정답을 만들지 못했거나 표시 기준보다 신뢰도가 낮았던 진단 세션을 HTTPS로 받습니다. 수신기는 Python 표준 라이브러리만 사용하며 Nginx 뒤의 `127.0.0.1:8765`에서 실행합니다.
...
- 저장 경로: `/var/lib/gta-report-receiver/YYYY-MM-DD/`
...
수신 ZIP은 `session.json`과 세션 JPEG만 허용합니다. 경로가 포함된 파일, 중복 이름, 비정상 JPEG, 50MB 초과 요청과 100MB 초과 압축 해제 크기는 거부합니다. Nginx는 요청 크기와 IP별 요청 속도를 제한하며 업로드 경로의 접근 로그는 남기지 않습니다.
