...
from gta_helper.models import AppState, DisplayMode, SolveResult
...
class HelperApp:
    ...
    def show_settings(self) -> None:
        ...
        ttk.Label(body, text="캡처 백엔드").grid(row=2, column=0, sticky="w", pady=4)
        backend = ttk.Combobox(body, state="readonly", width=22, values=["auto", "dxgi", "winrt"])
        backend.set(self.config.capture_backend)
        backend.grid(row=2, column=1, padx=8)
        ttk.Label(body, text="입력 프로필").grid(row=3, column=0, sticky="w", pady=4)
        profile = ttk.Combobox(body, state="readonly", width=22, values=list(INPUT_PROFILES))
        profile.set(self.config.input_profile)
        profile.grid(row=3, column=1, padx=8)
        ttk.Label(body, text="사용자 키 (위/아래/왼쪽/오른쪽/선택/뒤로)").grid(row=4, column=0, columnspan=2, sticky="w", pady=(8, 2))
        key_vars: dict[str, tk.StringVar] = {}
        for row, key in enumerate(("up", "down", "left", "right", "select", "back"), start=5):
        ...
        ttk.Button(body, text="저장", command=save).grid(row=11, column=1, sticky="e", pady=(10, 0))
